...
func Chapter8(width, height int) Scene {
	shapes := make([]*Shape, 0)
	...
	lightCount := 2.0
	light := PointLight{Point(-10, 10, -10), Color{1, 1, 1}.Scale(1.0 / lightCount)}
	light2 := PointLight{Point(-5, 5, -20), Color{0.75, 0.75, 0.75}.Scale(1.0 / lightCount)}

	camera := NCamera(width, height, math.Pi/3, Point(0, 1.5, -5), Point(0, 1, 0), Vector(0, 1, 0))

	return NScene([]PointLight{light, light2}, camera, shapes)
